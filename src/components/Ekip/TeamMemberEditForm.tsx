...
import { updateTeamMember, getTeamMemberById } from "@/lib/admin/action";
import {
  HiUser,
  HiIdentification,
  HiBriefcase,
...
export default function TeamMemberEditForm({
  ...
  useEffect(() => {
    const fetchMemberData = async () => {
      try {
        ...
        toast.error(error.message || "Ekip üyesi bilgileri yüklenemedi", {
          position: "top-right",
          autoClose: 5000,
        });
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await updateTeamMember(memberId, formData);
    ...
    return (
      <div className="flex justify-center items-center min-h-[60vh]">
        <div className="text-center">
          <VscLoading className="animate-spin text-4xl text-purple-600 mx-auto mb-4" />
          <p className="text-gray-600">Ekip üyesi bilgileri yükleniyor...</p>
        </div>
      </div>
    );
  }
      ...
      >
        <div className="flex flex-col items-center mb-2">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Ekip üyesi bilgilerini güncellemek için formu düzenleyin.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {(photoPreview || currentPhoto) && (
            <div className="flex flex-col gap-2">
              <label className="font-medium text-gray-700">
                {photoPreview ? "Yeni Fotoğraf Önizleme" : "Mevcut Fotoğraf"}
              </label>
              <div className="relative w-32 h-32 mx-auto">
                <img
                  ...
                  className="w-full h-full object-cover rounded-lg border-2 border-purple-300 shadow-md"
                />
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              <HiCamera className="text-lg text-purple-500" />
              {currentPhoto ? "Fotoğraf Değiştir" : "Fotoğraf Yükle"}
            </label>
        ...
        <div className="flex gap-4 mt-2">
          <button
            type="submit"
            disabled={loading}
            className="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 transition-all text-white p-3 rounded-lg font-semibold shadow-md disabled:opacity-60 disabled:cursor-not-allowed text-lg tracking-wide"
          >
            {loading ? (
