...
import { createSpecialFile } from "@/lib/special-files/action";
...
export default function AddSpecialFilePage({}: Props) {
  ...
  useEffect(() => {
    const fetchAuthors = async () => {
      try {
        const response = await availableAuthors();
  ...
  const handleSubmit = async (
    ...
    // Otomatik olarak ilk oluşturma kaydı ekle
    specialFileData.changes = "İlk oluşturma";

    try {
      const result = await createSpecialFile(specialFileData);
      if (result) {
        toast.success("Özel dosya başarıyla oluşturuldu!");
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 max-w-full mx-auto">
      <h1 className="text-2xl font-bold mb-6">Özel Dosya Ekle</h1>
            ...
            <div>
              ...
              {values.authorId && (
                <div className="text-xs text-gray-500 mt-1">
                  Seçili yazar:{" "}
                  {authors?.find((a: any) => a.id === values.authorId)
                    ?.fullName || "-"}
                </div>
              )}
            </div>
